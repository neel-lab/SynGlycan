function msdata_QA = quantitativeAnalysis(scannums,msdata,MS1tol,MS1tolUnit,...
    MS2tol,simscoretol,px,retlim,chnos,areayn,mergeyn,plotyn,distyn)
...
elseif ~(strcmpi(MS1tolUnit,{'ppm','Da'}))
    error('MATLAB:Glycomics Toolbox:ERRORUNIT','INCORRECT UNIT FOR MS1 MASS TOL');
end
...
for ii = 1:length(scannums)%scannumind = 1:length(allmslvl)
    ...
    if allmslvl(scannumind) > 1 % for all MSn scans
        ...
        if isfield(msdata,'theomassdist')
            theoisodist = msdata.theomassdist{scannumind}; % values saved from Averagine model
            if isempty(theoisodist)
                theoisodist = massDistribution(scannumind,precursorMz,allcharge(scannumind),allretime,allmslvl,allspectra,defaultmonoisodist,chnos); % calculate m/z values
            end
        else
            theoisodist = massDistribution(scannumind,precursorMz,allcharge(scannumind),allretime,allmslvl,allspectra,defaultmonoisodist,chnos); % calculate m/z values
        ...
        leftrtind = thisretimems1ind + 1; % initial left index for retention range
        rightrtind = thisretimems1ind; % initial right index for retention rangezerointcounter = 0;
        TICint = zeros(length(ms1spectra),1);
        zerointcounter = 0;
        while zerointcounter < zerointcounterlimit
            if leftrtind > 1
                leftrtind = leftrtind - 1;
                leftspec = ms1spectra{leftrtind};
                if distyn == 1 % if using isotopic mass distribution to calculate TIC
                    precpkind_dist = (abs(leftspec(:,1) - precursorMz) <= MS1tolC); % find precursor peaks
                    for oo = 1:o
                ...
                if any(precpkind)
                    TICint(leftrtind) = sum(leftspec(precpkind,2));
                    zerointcounter = 0;
                else
                    zerointcounter = zerointcounter + 1;
                end
            else
                zerointcounter = zerointcounterlimit; % stop immediately, return result
            end
        end
        zerointcounter = 0;
        while zerointcounter < zerointcounterlimit
            if rightrtind < length(ms1spectra)
                ...
                if distyn == 1 % if using isotopic mass distribution to calculate TIC
                    precpkind_dist = (abs(rightspec(:,1) - precursorMz) <= MS1tolC); % find precursor peaks
                    for oo = 1:o
                        aa = abs(rightspec(:,1) - massdist(oo,1)) <= MS1tolC;
                        precpkind_dist = [precpkind_dist,aa];
                    end
                    precpkind = any(precpkind_dist == 1,2);
                else % if using only precursor m/z to calculate TIC
                ...
                if any(precpkind)
                    TICint(rightrtind) = sum(rightspec(precpkind,2));
                    zerointcounter = 0;
                else
                    zerointcounter = zerointcounter + 1;
                end
        ...
        if any(TICint)
            leftrtind = max(1,find(TICint,1,'first') - 1);
            rightrtind = min(length(ms1spectra),find(TICint,1,'last') + 1);
            select_TIC = [ms1retime(leftrtind:rightrtind),TICint(leftrtind:rightrtind)]; % selecting area of TIC between left and right index
            avgms1drt = mean(diff(select_TIC(:,1)));
            if leftrtind == 1
                select_TIC = [[select_TIC(1,1) - avgms1drt,0];select_TIC];
            end
            if rightrtind == length(ms1spectra)
                select_TIC = [select_TIC;[select_TIC(end,1) + avgms1drt,0]];
            end
                ...
                elseif (thisretime > retmin(end)) && (thisretime <= select_TIC(end,1))
                    ...
                    for rr = 2:size(splits,1)
                        if (thisretime >= retmin(rr-1)) && (thisretime < retmin(rr))
                            ...
                            break
                        elseif rr == 2
                            retention_range{scannumind}(1,1) = 0;
                            retention_range{scannumind}(1,2) = retmin(rr - 1);
                        elseif rr == size(splits,1)
                            retention_range{scannumind}(1,1) = retmin(rr);
                            retention_range{scannumind}(1,2) = ms1retime(min(length(ms1retime),rightrtind + 1));
                        end
                    end
                end
            end
            ...
            if areayn == 1 % if desired, calculate area under TIC curve
                selectAUCdata_raw = calculateAUC(select_TIC,retention_range{scannumind});
                selectAUCdata(scannumind) = selectAUCdata_raw/theoisodist(1,2);
            end
            TICdata{scannumind} = select_TIC;
        else
            retention_range{scannumind} = [thisretime,thisretime];
            TICdata{scannumind} = [thisretime,0;thisretime,0];
        end
...
elseif idx <= defaultmonoisodist{end,1}(1,1) % if the precursor m/z is within the theoretical distribution
    ...
    if ~isempty(chnos)
        ...
        avggp = [0.024321959339170,0.476559048284606,0.001461698046926,0.012941719677186,0]; % [C H N O S]
    end
...
for mm = 1:length(aa)
    ...
    if length(row) > 6
        ...
        elseif ~(strcmpi(MS2tolUnit,{'ppm','Da'}))
            error('MATLAB:Glycomics Toolbox:ERRORUNIT','INCORRECT UNIT FOR MS2 MASS TOL');
        end
        lscn = length(scans);
    ...
    if ~isempty(sim_score1)
        ...
        for jj = 1:size(sim_score1(:,1),1)
            if sim_score1(jj,2) >= simscoretol
                ...
                [~,idx] = sort(combine_spec(:,1)); % sort combined spectrum based on m/z
                consensus{allscannum == scan_idx} = combine_spec(idx,:);
                allarea(allscannum == scan_idx) = allarea(allscannum == scan_idx) + allarea(allscannum == sim_score1(jj,1));
...
msdata.indexes = scan_indexes;
msdata.spectra = consensus;
msdata.originalAUC = msdata.parsedAUC;
msdata.parsedAUC = allarea;
