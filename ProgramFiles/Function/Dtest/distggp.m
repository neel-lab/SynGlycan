...
% DISTGGP: Parse the input sequence to get glycan(s) and peptide
%
% Syntax:
...
% inputformat: string, the format of input sequence. Currently "IUPAC" and
%     "SGP1" are supported.
%
% Output:
% gly: n x 1 cell array of strings, the glycan sequence of glycan/glycopeptide.
% pep: string, the peptide sequence, this output is blank when input is
%     glycan only.
...
%     This applies to SGP1.0 format only, this value is 1 for all other formats.
%
% Note:
% Input must be in a string.
% Input must consist of pure glycan(s) or contains only one
...
switch upper(inputformat)
    ...
    case 'SGP2'
        ...
        % TO BE COMPLETED - FUNCTION "BREAKGLYPEP" DOES NOT WORK FOR SGP2
    case 'IUPAC'
        thissgp = seqinput;
        thissgp = strtrim(thissgp);
        %% block user-customized info - to prevent intereferences
