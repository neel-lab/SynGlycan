...
for ii = 1:size(currentzone,1)
    currentlayer = currentzone(ii,:);
    for jj = 1:size(exsafezone,1)
        if currentlayer(1) >= exsafezone(jj,1) + 1 || currentlayer(1) <= exsafezone(jj,1) - 1
            vertcol(ii,jj) = 0;
        else
            vertoverlay(ii,jj) = 1 + exsafezone(jj,1) - currentlayer(1);
        end
        if currentlayer(3) <= exsafezone(jj,2) - 1 || currentlayer(2) >= exsafezone(jj,3) + 1
            horicol(ii,jj) = 0;
        else
            horioverlay(ii,jj) = 1 + min(exsafezone(jj,3) - currentlayer(2),currentlayer(3)-exsafezone(jj,2));
        end
    end
end
