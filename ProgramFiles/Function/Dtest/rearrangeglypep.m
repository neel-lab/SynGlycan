...
% glycosylpos: 1 x n double. Serial number of the amino acid that has
%     glycan attached to it.
% plotinfoout: n x 1 cell array. The output of CALCGLYPOS. Contains info for
%     painting program, 1st column contains the positioning info for each
%     monosaccharide in each glycan.
% options: structure. Global options. Here 'structspacing' and 'fontsize'
%     are used to estimate figure size and relative position of peptide 
%     and glycan(s).
%
% Output:
% plotinfoout: n x 1 cell array. Modified plot information after adjusting
%     glycan positions.
% peppos: 1 x n double. Position of each amino acid in the peptide
%     backbone. These are the x position of amino acid. They will be placed
%     at [x,-1] with "verticalalignment" set to "top".
%
% Note:
...
%     unit. The actual spacing in figure between AA's will then be decided
%     only by axis resolution, or to be exact, the figure size in pixels.
%     It is not recommended to use fontsize too big as overlapping text
...
if ~isempty(alladducttext) && strcmpi(alladducttext(end),' ')
    ...
    glycanLRspan = zeros(1,2);
    for i = 1:length(plotinfoout)
        tempmspos = plotinfoout{i}.mspos;
        glycanLRspan(1) = min([glycanLRspan(1),min(tempmspos(:,1))]);
        glycanLRspan(2) = max([glycanLRspan(2),max(tempmspos(:,1))]);
    end
    glypepLRspan = [min([peppos(1) - options.aaspacing,glycanLRspan(1) - options.structspacing]),...
        max([peppos(end) + options.aaspacing,glycanLRspan(2) + options.structspacing])];
    newbracketpos = [glypepLRspan(1) + 0.1,brackettop;...
        glypepLRspan(1),brackettop;...
        glypepLRspan(1),-1;...
        glypepLRspan(1) + .1,-1;...
        glypepLRspan(2) - .1,brackettop;...
        glypepLRspan(2),brackettop;...
        glypepLRspan(2),-1;...
        glypepLRspan(2) - .1,-1];
