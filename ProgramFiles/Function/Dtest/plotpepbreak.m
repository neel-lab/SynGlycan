...
%
% Syntax:
% pepbreaktcp = plotpepbreak(peppos,pepbreak,glycosylpos,options)
%
% Input:
% peppos: 1 x n double. Position of the amino acids in the backbone.
...
%
% Output:
% pepbreaktcp: 2 x 2 double. The lower left and upper right corner
%     coordinates.
%
% Note:
% N/A
...
if ~isfield(pepbreak,{'N','C'})
    ...
    if ~isempty(glycosylpos)
        fragmarkerlengthfactor = 2.5;
    else
        fragmarkerlengthfactor = 1.8;
    end
    ...
    delete(t);
    letterheight = diff(ylim)/fontheight*fragmarkerlengthfactor;
...
plot(ax,[peppos(glycosylpos(i)),peppos(glycosylpos(i))],[-.5,-options.monosacsize/2],'k','linewidth',options.bondwidth)
        ...
        for j = 1:numel(localpepbreaktyp)
            if strcmpi(localpepbreaktyp{j},'N')
                ...
                plot(ax,[posx,posx2],[posy(2),posy(2)],'k')
                                text(ax,posx,posy(2),char(double(localpepbreakcont{j})),'fontsize',options.fontsize,'HorizontalAlignment','right','VerticalAlignment','bottom','rotation',90)
%                 text(ax,posx,posy(2),char(double(localpepbreakcont{j})),'fontsize',options.fontsize,'VerticalAlignment','top','HorizontalAlignment','right','rotation',90)
            elseif strcmpi(localpepbreaktyp{j},'C')
                posx2 = posx + options.bondbreaksiglength/3;
                plot(ax,[posx,posx2],[posy(1),posy(1)],'k')
                text(ax,posx,posy(1),char(double(localpepbreakcont{j})),'fontsize',options.fontsize,'VerticalAlignment','top','rotation',90)
%                 text(ax,posx,posy(1),char(double(localpepbreakcont{j})),'fontsize',options.fontsize,'VerticalAlignment','bottom','rotation',90)
            end
        end
    end
