...
if stub ~= 0
    ...
    if any(ismember(PMroot,allparent))
        ...
        for ii = length(stubroot):-1:1
            instub = PMroot(stubroot(ii));
            tempPMSstruct = pmposstorage(stubroot(ii),:);
            ...
            if ~isempty(upperpmpos)
                ...
                mspos(pmindstorage{stubroot(ii),1},:) = upperpmpos;
            end
            safezone(pmindstorage{stubroot(ii),1}) = 1;
            ...
            if ~isempty(lowerpmpos)
                ...
                mspos(pmindstorage{stubroot(ii),2},:) = lowerpmpos;
            end
            safezone(pmindstorage{stubroot(ii),2}) = 1;
        end
    end
end