...
% GETSEQOPTIONS: extract local options and option values from IUPAC
%     sequence of glycan or peptide.
%
% Syntax:
% [attachment,cleanseq] = getseqoptions(inseq,mode)
%
% Input:
% inseq: string. IUPAC condensed glycan/glycopeptide sequence.
% mode: 'g' for glycan or 'p' for peptide.
%
% Output:
% attachment: structure. Field names are option names, values are 1 x 2
%     cell arrays, 1st element is option value, 2nd is which monosac.
%
% Note:
...
% C                                            Text that appear at the center of  monosac. symbol.
% P                                            Monosac. that is perpendicular to main structure, 
%                                                    orientation is automatically determined.
% PR                                         Perpendicular monosac. placed at clockwise
%                                                    perpendicular position.
% PL                                          Perpendicular monosac. placed at counterclockwise
%                                                    perpendicular position.
...
% DASH                                   Bond shown as regular dashed line.
% DASHB                                 Bond shown as perpendicular dashed line.
% WAVY                                   Bond shown as wavy line.
% WEDGE                                Bond shown as a wedge.
% DOUBLE                               Bond shown as double line.
% TRIPLE                                  Bond shown as triple line.
...
switch upper(mode)
    case 'G'
        [opttype,opttypestart,opttypeend] = regexp(inseq,...
            DrawGlycanPara.regexp_option,'match','start','end');
        [optcontent,optcontentstart,optcontentend] = regexp(inseq,...
        ...
        allopt = [opttypestart,optcontentstart;opttypeend,optcontentend];
        % matching options with contents
        if ~isempty(allopt)
            ...
            if ~isempty(opttype)
                glymodinfo = [DrawGlycanPara.intglymodinfo,...
                    DrawGlycanPara.extglymodinfo,DrawGlycanPara.intglybondinfo,...
                    DrawGlycanPara.glyidentityinfo,DrawGlycanPara.ambiglyinfo];
            ...
            for i = 1:length(optcontentstart)
                cleanseq(optcontentstart(i):optcontentend(i)) = blanks(optcontentend(i)-...
                    optcontentstart(i)+1);
            end
            [mslinkagestart,mslinkageend] = regexp(cleanseq,...
                DrawGlycanPara.regexp_monosaclinkage,'start','end');  % based on ()
            for i = length(mslinkagestart):-1:1
                if mslinkageend(i) - mslinkagestart(i) > 1
                    ...
                    localopt = allopt(:,allopt(1,:) > mslinkagestart(i) & ...
                        allopt(2,:) < mslinkageend(i));
                    ...
                    cleanseq = [cleanseq(1:mslinkagestart(i)),templinkageseq,...
                        cleanseq(mslinkageend(i):end)];
                end
            end
        else
            attachment = [];
    ...
    case 'P'
        [optcontentstart,optcontentend,optcontent] = regexp(inseq,...
            DrawGlycanPara.regexp_optionvalue,'start','end','match');  % clear " "
        for i = 1:length(optcontentstart)
            cleanseq(optcontentstart(i):optcontentend(i)) = blanks(...
                optcontentend(i)-optcontentstart(i)+1);
        end
        [opttypestart,opttypeend,opttype] = regexp(cleanseq,...
            DrawGlycanPara.regexp_option,'start','end','match');
        [alltypestart,alltypeend,alltype] = regexp(cleanseq,...
            DrawGlycanPara.regexp_monosaclinkage,'start','end','match');
        for i = 1:length(alltypestart)
            cleanseq(alltypestart(i):alltypeend(i)) = blanks(alltypeend(i)-alltypestart(i)+1);
        end
        [ngmodstart,ngmodend,ngmodtype] = regexp(cleanseq,...
            '<(.*?)>','start','end','match');
        for i = 1:length(ngmodstart)
            cleanseq(ngmodstart(i):ngmodend(i)) = blanks(ngmodend(i)-ngmodstart(i)+1);
        end
        [aaIndex,aaend,aa] = regexp(cleanseq,DrawGlycanPara.regexp_AA,...
            'start','end','match');
