function statusreporthandles = scoreAllSpectra(msdata,input)
...
% Description:
% This program reads scoreoptions from the GUI
% It also reads the SASSO_trigger file
% It the sets the fragmentation modes for HCD, CID and EThcD
% It then defines the serach space (glypep) for N-glycan, O-glycan and peptide only search
% Following is the preserach module for N-glycans, the main serach for
% N-glycans (searchspectra) and then the main search for O-glycans (searchspectra_O)
...
scoreoptions.keeptopn = GlycoPATConstant.KeepTopOglynum;
scoreoptions.nummindiagionfragmode=GlycoPATConstant.fbi_nummmindiagionfragmode;
...
scoreoptions.analyzeglycan = input.analyzewhichglycan;
...
numpw = input.doparacomp; % doparallel if numpw>1
allfragments_presearch = input.allfragments_presearch;
outputfname = input.outputfname;

% Special options
scoreoptions.doparallel = false;  % may not be used
...
triggerdata = [];
if scoreoptions.isHCDtrigger  % HCD trigger - supplemental activation experiments
    ...
    colkeepind = ismember(upper(colnames),upper(scoreoptions.analyzefragmode));
    triggerdata.colnames = triggerdata.colnames(colkeepind);
    SASSO = triggerdata.SASSO;
    SASSO_MSn = SASSO(:,1:end-2);
    [~,unitriggerscanind,~] = unique(triggerdata.SASSO(:,1),'stable');
    triggerdata.SASSO = [SASSO_MSn(unitriggerscanind,colkeepind),...
        SASSO(unitriggerscanind,end-1:end)];
end
...
% 1 - glycopeptide only
% 2 - peptide only
if ismember(input.analyzetgt,[1,2])
    ...
    switch input.analyzetgt
        case 1  % Glycopeptide only
            switch upper(scoreoptions.analyzeglycan)
                case 'N'
                    for ii = 1:length(glypep)
                        ...
                        for jj = 1:length(keepglypep)
                            ...
                            for kk = 1:length(ptmsites)
                                ptmind(kk) = str2double(ptmsites{kk}(2:end-1));
                            end
                ...
                case 'O'
                    glycanptmtxt = ['{',num2str(glycanind),'}'];
                    for ii = 1:length(glypep)
                        thisprot = glypep{ii};
                        keepglypep = false(size(thisprot));
                        % Identify the PTM marker on each glypep, if peptide does
                        ...
                        for jj = 1:length(keepglypep)
                            ...
                            for kk = 1:length(oglycosylationpos)
                                ...
                                if (thisoglypos + 1 <= length(glypepseq)) && (~strcmpi(tempglypepseq(thisoglypos + 1),'{'))
                                    tempglypepseq = [tempglypepseq(1:thisoglypos),glycanptmtxt,tempglypepseq(thisoglypos+1:end)];
                                end
                            end
                            thisprot{jj} = tempglypepseq;
                            if scoreoptions.excludeNglysequon
    ...
    if strcmpi(scoreoptions.analyzeglycan,'N')
        ...
        switch upper(scoreoptions.analyzeglycan)
            case 'N'
                ...
                searchspectra(input,msdata,triggerdata,allpepdata_pres,statusreporthandles,...
                    todo{2},allfragments_presearch,scoreoptions_presearch);
                
                % Collect and analyze data
                [keepwhichprotein,~] = analyzepresearchdata(input,triggerdata,allpepdata_pres,...
                ...
                searchspectra(input,msdata,triggerdata,allpepdata_filtered,statusreporthandles,...
                    todo{2},allfragments,scoreoptions);
                [~,f,ext] = fileparts(input.outputfname);
                allfragmentssavepath = fullfile(input.outputdir,[f,'_allfragments',ext]);
                save(allfragmentssavepath,'allfragments');
                findbestisomers(input,msdata,input.allfragments,2,allpepdata_filtered);
            case 'O'
                error('PreSearch not supported for O-Glycan in current version');
%                 searchspectra_O(input,msdata,triggerdata,allpepdata,statusreporthandles,...
%                     todo{2},allfragments,scoreoptions);
%                 [~,f,ext] = fileparts(input.outputfname);
%                 allfragmentssavepath = fullfile(input.outputdir,[f,'_allfragments',ext]);
%                 save(allfragmentssavepath,'allfragments');
    ...
    if ~isempty(todo{2})        % main search with full library
        switch upper(input.analyzewhichglycan)
            case 'N'
                ...
                if input.analyzetgt == 1
                    findbestisomers(input,msdata,input.allfragments,2,allpepdata);
                end
            case 'O'
                searchspectra_O(input,msdata,triggerdata,allpepdata,statusreporthandles,...
                    todo{2},allfragments,scoreoptions);
