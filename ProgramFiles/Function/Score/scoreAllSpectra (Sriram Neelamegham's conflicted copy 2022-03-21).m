...
if input.dohcdtrigger  % HCD trigger - supplemental activation experiments
    ...
    colkeepind = ismember(upper(colnames),upper(input.fragmode));
    ...
    if  ~isempty(todo{1})
        ...
        switch upper(input.analyzewhichglycan)
            case 'N'
                ...
                fbioptions.diagnionlogic = 1;
                findbestisomers(input,msdata,input.allfragments,2,fbioptions);
            case 'O'
        ...
        switch upper(input.analyzewhichglycan)
            case 'N'
                searchspectra(input,msdata,triggerdata,allpepdata,statusreporthandles,...
                    todo{2},input.allfragments,scoreoptions);
                fbioptions.nummindiagionfragmode = 1;
                fbioptions.allpepdata = allpepdata;
                fbioptions.diagnionlogic = 2;
                findbestisomers(input,msdata,input.allfragments,2,fbioptions);
            case 'O'
