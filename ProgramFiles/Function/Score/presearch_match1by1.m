function [spectrascores,stopsignal] = presearch_match1by1(sgp,sgpmass,theofrag,proteinID,scannum,...
...
if ~stopsignal    % default is zero
    if size(spectrum,1) >= 5
        ...
        ioncoverage = sum(~cellfun(@isempty,theores.ionmatchindex))/length(theores.ionmatchindex);
        Top10 = calculatetop10match(spectrum,theores.peakmatchindex,2,400);
%         if ioncoverage < .01 || Top10 < 2  % HUMAN SERUM DATA SUGGESTS 1.2% / 2 AS CRITERIA
%             proceed = false;
%             stopsignal = true;
%         end
        if proceed
            ...
            for i = 1:length(fragAAind)
                fragAAind{i} = theofrag(i).unitindex{1};
                fragglyind{i} = theofrag(i).unitindex{2};
                fragmodind{i} = theofrag(i).unitindex{3};
            end
            ...
            if proceed
                ...
                for i = 1:nDecoy
                    ...
                    decoyres = calcithscore(spectrum,decoyfragmz,charge,tol,tolunit,3,cisoptions);
